...
function extractInlineScript(html) {
    ...
    if (!scripts.length) {
        throw new Error('index.html 找不到內嵌 JavaScript');
    }
...
async function runBaselineCase(name, want, html) {
    ...
    const data = new Uint8Array(
        await readFile(
            new URL(want.file, import.meta.url),
        ),
    );
    ...
    // 新版共同規則：
    // 總經費列若存在，必須全部標示 excluded=true。
    const totalCostRows = rows.filter(
        r => r.level === '總經費',
    );

    const activeTotalCostRows = totalCostRows.filter(
        r => r.excluded !== true,
    );
    ...
    if (activeTotalCostRows.length) {
        errors.push(
            `總經費排除旗標錯誤：`
            + `${activeTotalCostRows.length} 列未標示 excluded=true`,
        );
    }
    ...
    if (leakedTotalCostDetails.length) {
        errors.push(
            `總經費誤入一般明細：`
            + `${leakedTotalCostDetails.length} 列`,
        );
    }
...
async function runSocialCase(html) {
    ...
    const data = new Uint8Array(
        await readFile(
            new URL(SOCIAL_CASE.file, import.meta.url),
        ),
    );
    ...
    for (const amount of expectedTotalCosts) {
        const found = totalCostRows.some(
            r => +r.amount === amount,
        );
    ...
    // 取得 index.html 內部的自動修復紀錄。
    // 頂層 let 不會直接成為 ctx 屬性，
    // 因此需以 vm.runInContext() 取得。
    const repairs = vm.runInContext(
        '_repairs',
        ctx,
    );
